...
def train_GBDT2NN(args, num_data, plot_title, key, pre_data, trained_gbdt_model=None, kd_type='emb'):
    ...
    # print(concate_test_x, concate_train_x)
    ...
    for seed in args.seeds:
        ...
        pre_data_ = np.concatenate([pre_data, np.zeros((pre_data.shape[0],1), dtype=np.float32)], axis=-1)
        y_pre = evalute(pre_data_, gbdt2nn_model, 1)

    return gbdt2nn_model, opt, metric, y_pre
...
def train_D1(args, num_data, cate_data, plot_title, key="", trained_gbdt_model=None):
    ...
    for seed in args.seeds:
        ...
        opt = AdamW(deepgbm_model.parameters(), lr=args.lr, weight_decay=args.l2_reg, amsgrad=False, model_decay_opt=deepgbm_model, weight_decay_opt=args.l2_reg_opt, key_opt='deepfm')
        # TraisavenWithLog(args, plot_title+'seed'+str(seed), trn_pred, train_y, None,
