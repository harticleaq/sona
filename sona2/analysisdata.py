...
class DataAnalysis():
    def __init__(self, url, trans_aisle=False):
        self.sameple_rate,self.sigs_ = wf.read(url)
        self.name = url
        if trans_aisle:
            self.sigs_ = self.sigs_.reshape(-1)
    ...
    def get_model_para(self):
        item = [self.TIME_MAX,self.TIME_MIN,self.TIME_MEDIAN,self.TIME_MEAN,self.TIME_VAR,self.TIME_STD,
                self.TIME_KURT, self.TIME_SKEW,self.TIME_RMS,self.TIME_KURT_FACTOR,
                self.TIME_PULSE_FACTOR,self.TIME_MARGIN_FACTOR,self.FREQ_MAX,
                self.FREQ_MIN,self.FREQ_MEDIAN,self.FREQ_MEAN,self.FREQ_VAR,self.FREQ_STD]
        return item
    # 3,4,5,6,9,10,11,12,14,15,16,17,18,19,20,21,23,24
